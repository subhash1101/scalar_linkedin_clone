...
function CommentIcon() {
    return (
        <svg width="14" height="18" viewBox="0 0 512 512" fill="currentColor">
        ...
        <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-start', padding: '4px 12px' }}>
                ...
                >
                    ...
                    >
                        {activeReaction ? (
                            <><span style={{ display: 'flex', alignItems: 'center', width: 18, height: 18 }}>{activeReaction.icon}</span> {post.like_count > 0 && <span>{post.like_count}</span>}</>
                        ) : (
                            <><ThumbsUpIcon /> {post.like_count > 0 && <span>{post.like_count}</span>}</>
                        )}
                    </button>
                            ...
                            >
                                {REACTIONS.map(r => (
                                    <button
                                        ...
                                        title={r.label}
                                        style={{ background: 'none', border: 'none', cursor: 'pointer', transition: 'transform 0.1s', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 4 }}
                ...
                <button
                    ...
                    style={{ ...postActionStyle, justifyContent: 'center' }}
                    onMouseOver={e => ((e.currentTarget as HTMLElement).style.backgroundColor = '#ebebeb')}
                    onMouseOut={e => ((e.currentTarget as HTMLElement).style.backgroundColor = '')}
                >
                    <CommentIcon /> {post.comment_count > 0 && <span>{post.comment_count}</span>}
                </button>
                ...
                <button
                    ...
                    onMouseOut={e => ((e.currentTarget as HTMLElement).style.backgroundColor = '')}
                >
                    <RetweetIcon /> {post.repost_count > 0 && <span>{post.repost_count}</span>}
                </button>
